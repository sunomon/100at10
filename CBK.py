...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            ...
            target7_price = get_target7_price("KRW-CBK", 0.7)
            target8_price = get_target8_price("KRW-CBK", 0.8)
            target9_price = get_target9_price("KRW-CBK", 0.9)
            target10_price = get_target10_price("KRW-CBK", 1)
            ...
            if target7_price <= current_price < target7_price*1.02 and target7_price*1.1 <= predicted_close_price:
                if krw >= 1000000 and cbk < 10000/(target7_price*1.02):
                    upbit.buy_market_order("KRW-CBK", 1000000)
                if 5000 < krw < 1000000 and cbk < 10000/(target7_price*1.02):
                    upbit.buy_market_order("KRW-CBK", krw*0.9995)
            if target8_price <= current_price < target8_price*1.02 and target8_price*1.1 <= predicted_close_price:
                if krw >= 1000000 and cbk < 10000/(target8_price*1.02):
                    upbit.buy_market_order("KRW-CBK", 1000000)
                if 5000 < krw < 1000000 and cbk < 10000/(target8_price*1.02):
                    upbit.buy_market_order("KRW-CBK", krw*0.9995)
            if target9_price <= current_price < target9_price*1.02 and target9_price*1.1 <= predicted_close_price:
                if krw >= 1000000 and cbk < 10000/(target9_price*1.02):
                    upbit.buy_market_order("KRW-CBK", 1000000)
                if 5000 < krw < 1000000 and cbk < 10000/(target9_price*1.02):
                    upbit.buy_market_order("KRW-CBK", krw*0.9995)
            if target10_price <= current_price < target10_price*1.02 and target10_price*1.1 <= predicted_close_price:
                if krw >= 1000000 and cbk < 10000/(target10_price*1.02):
                    upbit.buy_market_order("KRW-CBK", 1000000)
                if 5000 < krw < 1000000 and cbk < 10000/(target10_price*1.02):
                    upbit.buy_market_order("KRW-CBK", krw*0.9995)
            if cbk > 1000000*1.001*1.5/current_price:
            ...
            cbk = get_balance("CBK")
            current_price = get_current_price("KRW-CBK")
            if cbk > 1000000*1.001*1.05/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-CBK", cbk*0.9995)  
        else:
            cbk = get_balance("CBK")
            current_price = get_current_price("KRW-CBK")
