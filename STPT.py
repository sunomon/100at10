...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            ...
            target7_price = get_target7_price("KRW-STPT", 0.7)
            target8_price = get_target8_price("KRW-STPT", 0.8)
            target9_price = get_target9_price("KRW-STPT", 0.9)
            target10_price = get_target10_price("KRW-STPT", 1)
            ...
            if target10_price <= current_price < target10_price*1.02 and target10_price*1.1 <= predicted_close_price:
                ...
                if 5000 < krw < 1000000 and stpt < 10000/(target10_price*1.02):
                    upbit.buy_market_order("KRW-STPT", krw*0.9995)
            if stpt > 1000000*1.001*1.5/current_price:
            ...
            stpt = get_balance("STPT")
            current_price = get_current_price("KRW-STPT")
            if stpt > 1000000*1.001*1.05/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-STPT", stpt*0.9995)  
        else:
            stpt = get_balance("STPT")
            current_price = get_current_price("KRW-STPT")
